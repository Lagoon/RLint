...
If credentials present on config file, or first connection to Lagoon® fails, a Rlint::SanityProblem exception will be raised on application start.

h3. ClientError

If any problem was detected on client request (ex: wrong attributes sent) Lagoon® will sent a client error on http status code. Rlint will raise a Rlint::ClientError exception. You can catch this and modify it behavior in ApplicationController.
